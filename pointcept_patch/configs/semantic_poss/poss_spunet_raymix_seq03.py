...
# SemanticPOSS seq03 protocol — PROPOSED: SpUNet + Ray-Consistent Instance Transplantation (RayMix).
# IDENTICAL backbone/recipe/seed to poss_spunet_polarmix_seq03.py; ONLY the train dataset changes
# (SemanticPOSSPolarMixDataset -> SemanticPOSSRayMixDataset). This isolates the instance mechanism:
# naive rotate-paste  ->  ray-consistent (support + occlusion + validity) transplantation.
# Target to beat: our own SpUNet+PolarMix = 60.88 best-val mIoU (and published PolarMix 58.6).
batch_size = 12
mix_prob = 0.8
empty_cache = False
enable_amp = True
...
data = dict(
    ...
    train=dict(type="SemanticPOSSRayMixDataset", split="train", data_root=data_root, transform=_tf_train,
